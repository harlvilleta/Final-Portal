...
export default function FoundItemRecords() {
  ...
  useEffect(() => {
    const fetchFoundItems = async () => {
      try {
        const foundSnap = await getDocs(query(collection(db, 'found_items'), orderBy('createdAt', 'desc')));
        setFoundItems(foundSnap.docs.map(doc => ({ id: doc.id, ...doc.data() })));
      } catch (error) {
        console.error("Error fetching found items:", error);
        setSnackbar({ 
          open: true, 
          message: "Error loading found items: " + error.message, 
          severity: "error" 
        });
      }
    };
  ...
  return (
    <Box sx={{ pt: { xs: 2, sm: 3 }, pl: { xs: 2, sm: 3, md: 4 }, pr: { xs: 2, sm: 3, md: 4 } }}>
      ...
      <Box sx={{ mb: 3 }}>
        <TextField
          fullWidth
          placeholder="Search found items by name, description, location, or person..."
          value={searchTerm}
      ...
      <Grid container spacing={2}>
        ...
        <Grid item xs={12} md={6}>
          <Paper sx={{ 
            ...
            <Typography variant="h6" gutterBottom sx={{ color: theme.palette.mode === 'dark' ? '#ffffff' : '#800000' }}>
              Active Found Items ({activeItems.length})
            </Typography>
        ...
        <Grid item xs={12} md={6}>
          <Paper sx={{ 
            ...
            <Typography variant="h6" gutterBottom sx={{ color: theme.palette.mode === 'dark' ? '#ffffff' : '#800000' }}>
              Resolved Found Items ({resolvedItems.length})
            </Typography>
